...
function plainProof(signal: Evidence, couponCode: string) {
  if (signal.kind === "device") return "They used the same device.";
  if (signal.kind === "payment") return "They used the same payment method.";
  if (signal.kind === "address") return "They used the same delivery address.";
  if (signal.kind === "ip") return "They used the same internet connection.";
  if (signal.kind === "timing") return `They used ${couponCode} within a short time of each other.`;
  return "They shared another detail we need to check.";
}

export function CaseDetailView({ ring }: { ring: RingCase }) {
  const [activeTab, setActiveTab] = useState<Tab>("graph");
  const members = useMemo(() => ring.accountIds.map((id) => accounts.find((account) => account.id === id)).filter((account): account is Account => Boolean(account)), [ring.accountIds]);
  const signals = ring.evidence.filter((evidence) => evidence.accountIds.length > 1);
  const exposureByAccount = useMemo(() => new Map(redemptions.filter((item) => ring.accountIds.includes(item.accountId)).map((item) => [item.accountId, item])), [ring.accountIds]);

  return <main className="case-page">
    <header className="case-header"><Link href="/dashboard?guest=1">← Back to dashboard</Link><div><span>Potential offer loss</span><b>₹{ring.exposureInr.toLocaleString("en-IN")}</b></div></header>
    <div className="case-shell">
      <nav className="case-tabs" aria-label="Case views">
        {(["graph", "accounts", "evidence"] as Tab[]).map((tab) => <button key={tab} type="button" className={activeTab === tab ? "active" : ""} onClick={() => setActiveTab(tab)}>{tab === "graph" ? "Map" : tab === "accounts" ? `Customers (${members.length})` : "Why we are checking"}</button>)}
      </nav>

      {activeTab === "graph" && <section className="case-graph-card">
        <ul className="case-proof-list">{signals.map((signal, index) => <li key={`${signal.kind}-${index}`}>{plainProof(signal, ring.couponCode)}</li>)}</ul>
        <div className="circle-graph" role="img" aria-label={`Circular evidence map for case ${ring.id}, with ${members.length} accounts and ${signals.length} shared signals`}>
          ...
          <div className="circle-center"><span>CASE</span><b>{ring.id}</b><small>{ring.couponCode}</small></div>
        </div>
      </section>}

      {activeTab === "accounts" && <section className="case-data-card"><div className="case-data-heading"><h2>Customers in this group</h2><p>These customers used the same offer.</p></div><div className="case-table-scroll"><table><thead><tr><th>Customer</th><th>Joined</th><th>Offer used</th></tr></thead><tbody>{members.map((member) => { const redemption = exposureByAccount.get(member.id); return <tr key={member.id}><td><b>{member.id}</b></td><td>{formatDate(member.createdAt)}</td><td>{redemption ? <><b>{redemption.code}</b><small> ₹{redemption.discountInr.toLocaleString("en-IN")}</small></> : "—"}</td></tr>; })}</tbody></table></div></section>}

      {activeTab === "evidence" && <section className="case-evidence-grid">{signals.map((signal, index) => { const style = styles[signal.kind]; return <article key={`${signal.kind}-${index}`}><div><span style={{ backgroundColor: style.color }} />Reason to check</div><h2>{plainProof(signal, ring.couponCode)}</h2><p>This applies to all {signal.accountIds.length} customers in this group.</p></article>; })}</section>}

      <aside className="case-caution"><b>Note:</b> This is a reason to check the group. It does not prove that anyone did anything wrong.</aside>
    </div>
  </main>;
}
