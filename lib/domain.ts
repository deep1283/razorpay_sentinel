...
export type DashboardSnapshot = { generatedAt: string; source: "demo" | "live"; cases: RingCase[]; metrics: { precision: number; recall: number; f1: number; falsePositiveReviewRate: number; heldOutScenarios: number; developmentScenarios: number; reviewThreshold: number; truePositives: number; falsePositives: number; falseNegatives: number; trueNegatives: number; falsePositiveReviewCostInr: number } };
